...
public class CustomCircleProgress extends ProgressBar{
    ...
    private static final int PROGRESS_REACHED_COLOR = 0XFFFF6D24;//进度条的颜色
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        if(sweepAngle != 0)
        canvas.drawArc(new RectF(0, 0, mRadius * 2, mRadius *2), -90, sweepAngle-2*(float)Math.toDegrees(Math.asin(3.0/(mRadius*1.0))), false, mPaint);//drawArc：绘制圆弧
       // canvas.drawArc(new RectF(0, 0, mRadius * 2, mRadius *2), -90, sweepAngle, false, mPaint);//drawArc：绘制圆弧

        mPaint.setStrokeWidth(1);
        mPaint.setStyle(Paint.Style.STROKE);
        if(sweepAngle != 0)
        canvas.drawCircle((float)(mRadius+mRadius*Math.sin(Math.toRadians(sweepAngle))), (float)(mRadius-mRadius*Math.cos(Math.toRadians(sweepAngle))), 6, mPaint);
        mPaint.setStrokeWidth(mDefaultHeight);
        mPaint.setStrokeWidth(mReachedHeight);
        ...
        if(mStatus == Status.End){//未开始状态，画笔填充三角形
            ...
            //设置颜色
            mPaint.setColor(Color.parseColor("#FF6D24"));
            //画三角形
